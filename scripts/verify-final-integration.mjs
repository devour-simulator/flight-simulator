...
const factory=new Function('state','activeAircraft','activeWeather','selectedAirport','$','displayDistanceKm','THREE','calculateQ400VSpeeds','fmcSpeeds',`${source.slice(start,end)};return dispatchPerformance;`);
const calculate=factory(state,aircraft,{rain:0,gust:4},()=>airport,selector=>({value:selector==='#runwaySelect'?'09':''}),meters=>meters*8/1000,{MathUtils:{clamp:(value,min,max)=>Math.max(min,Math.min(max,value))}},()=>({v1:90,vr:94,v2:102,vref:113}),()=>({v1:130,vr:135,v2:142}));
